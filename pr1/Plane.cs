...
namespace pr1
{
    public class Plane
    {
        ...
        public float[] GetZoomRelativeCursorPosition(Window window, int x, int y)
        {
            ...
            if (!Window.Windoed)
            {
                xpos = -1 + (-window.Location.X + x) / (float) window.Width * 2;
                ypos = 1 - (-window.Location.Y + y) / (float) window.Height * 2;
            }
            else
            {
                xpos = -1 + (-window.Location.X + x - 8) / (float) window.Width * 2;
                ypos = 1 - (-window.Location.Y + y - 30) / (float) window.Height * 2;
            }
